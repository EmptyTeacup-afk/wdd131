...
function search() {
    ...
    let filterRecipes = recipes.filter(function(recipes) {
        return (
            recipes.name.toLowerCase().includes(recipeQuery) ||
            recipes.description.toLowerCase().includes(recipeQuery) ||
            recipes.tags.find((tag) => tag.toLowerCase().includes(recipeQuery))
        )
    });
...
/*Setup templates for dynamic html*/
...
function ratingTemplate(rating) {
    ...
    for (let i = 1; i <= 5; i++) {
        if (i <= rating) {
            ...
            html += `<span aria-hidden="true" class="icon-empty star">☆</span>`;
        }
    }
    
    // Close the rating span AFTER the loop finishes
    html += `</span>`;
    return html;
}
...
function renderRecipe(recipes) {
    let html = recipeTemplate(recipes);
